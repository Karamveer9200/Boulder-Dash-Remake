...
public class Main extends Application {
	...
	Element [][] elementGrid = new Element[GRID_LENGTH][GRID_WIDTH];
	...
	public void processKeyEvent(KeyEvent event) {
		...
		switch (event.getCode()) {
			case RIGHT:
				if (playerX < GRID_WIDTH - 1 && elementGrid[playerY][playerX + 1].isCanBeEntered()) {
					// Right key was pressed. So move the player right by one cell.
					elementGrid[playerY][playerX] = new Path(playerY,playerX);
					elementGrid[playerY][playerX + 1] = player;
				}
				break;
			case LEFT:
				if (playerX > 0 && elementGrid[playerY][playerX - 1].isCanBeEntered()) {
				...
				break;
			case UP:
				if (playerY > 0 && elementGrid[playerY - 1][playerX].isCanBeEntered()) {
					// Up key was pressed. So move the player up by one cell.
					elementGrid[playerY][playerX] = new Path(playerY,playerX);
					elementGrid[playerY - 1][playerX] = player;
				}
				break;
			case DOWN:
				if (playerY < GRID_LENGTH - 1 && elementGrid[playerY + 1][playerX].isCanBeEntered()) {
	...
	public void drawGame() {
		...
		for(int row = 0; row < elementGrid.length; row++) {
			for(int column =0; column < elementGrid[row].length; column++) {
				Element currentElement = elementGrid[row][column];
				if(currentElement.getName().equals("Player")) {
					playerX= column;
					playerY= row;
				}
				gc.drawImage(currentElement.getImage(), column * GRID_CELL_WIDTH, row * GRID_CELL_HEIGHT);
			}
		}
//		gc.drawImage(playerImage, playerX * GRID_CELL_WIDTH, playerY * GRID_CELL_HEIGHT);
	}

	public void readInteger2DArray(int[][] map) {
		for (int row = 0; row < map.length; row++) {
			for (int column = 0; column < map[row].length; column++) {
				if(gridTest[row][column] == 0) {
					elementGrid[row][column] = new Path(row,column);
				}
				else if(gridTest[row][column] == 1) {
					elementGrid[row][column] = new Dirt(row,column);
				}
				else if (gridTest[row][column] == 2) {
					elementGrid[row][column] = player;
				}
				else if (gridTest[row][column] == 3) {
					elementGrid[row][column] = new NormalWall(row,column);
				}
			}
		}
	}
